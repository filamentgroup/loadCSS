...
asyncTest( "loadCSS loads a CSS file with a relative path", function() {
	expect( 1 );
	var ss = loadCSS( "../qunit/files/test.css" );
	onloadCSS( ss, function() {
		ok( "stylesheet loaded successfully" );
		start();
	} );
...
asyncTest( "loadCSS loads a CSS file with specific attributes", function() {
	...
	onloadCSS( ss, function() {
		...
		equal(
			ss.title,
			attributes.title,
			"'title' attribute should be '" + attributes.title + "'"
		);

		equal(
			ss.type,
			attributes.type,
			"'type' attribute should be '" + attributes.type + "'"
		);
...
asyncTest( "onloadCSS callback fires after css is loaded", function() {
	expect( 1 );
	var getStyles = window.getComputedStyle ? function( node ) {
